...
void thrustControl(float current_pitch, float current_roll, float angVelX, float angVelY, float angVelZ) {
    ...
    // If both pitch and roll errors are within the threshold, no thrust is applied
    ...
    // Factor angular velocity into pitch and roll corrections
    float pitch_adjusted = pitch_correction - (angVelX);
    float roll_adjusted = roll_correction - (angVelY);

    // Apply corrections based on errors and angular velocity
    if (abs(pitch_error) > threshold) {
        if (pitch_adjusted > 0) {
            back_thrust = PSI; // Thrust on back
        } else {
            front_thrust = PSI; // Thrust on front
        }
    }

    if (abs(roll_error) > threshold) {
        if (roll_adjusted > 0) {
            right_thrust = PSI; // Thrust on right
        } else {
            left_thrust = PSI; // Thrust on left
        }
    }
